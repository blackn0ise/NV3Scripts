...
public class OptionsMenu : MonoBehaviour
{
	...
	[SerializeField] private Toggle middleToggle = default;
	[SerializeField] private Toggle gauntletsToggle = default;
	...
	IEnumerator BuildOptions()
	{
		...
		BuildPPToggle();
		BuildMiddleToggle();
		BuildGauntletsToggle();
	}

	private void BuildMiddleToggle()
	{
		middleToggle.gameObject.SetActive(gops.GetSave().highScore > 100);
		middleToggle.isOn = gops.GetSave().startInMiddle;
	}

	private void BuildGauntletsToggle()
	{
		gauntletsToggle.gameObject.SetActive(gops.GetSave().hasFoundGauntlets);
		gauntletsToggle.isOn = gops.GetSave().startWithGauntlets;
	}
	...
	public void SetStartInMiddle(bool value)
	{
		gops.GetSave().startInMiddle = value;
		middleToggle.isOn = gops.GetSave().startInMiddle;
	}

	public void SetStartWithGauntlets(bool value)
	{
		gops.GetSave().startWithGauntlets = value;
		gauntletsToggle.isOn = gops.GetSave().startWithGauntlets;
	}

...
public class Detrilight : MonoBehaviour, IMaterialChanger
{
	...
	void Start()
	{
		...
		wb = FindObjectOfType<CorporealShrine>();
	...
	//			break;
	//	}
	//	instance = Instantiate(effect, pos.position, altar.transform.rotation);
	//	Destroy(instance, 2f);
	...
	{
		if (go.GetComponent<CorporealShrine>())
		{
			for (int i = 0; i < go.GetComponent<CorporealShrine>().GetOriginalMaterials().Count; i++)
			{
				go.GetComponent<CorporealShrine>().GetComponentsInChildren<MeshRenderer>()[i].material = go.GetComponent<CorporealShrine>().GetOriginalMaterials()[i];
			}
		}

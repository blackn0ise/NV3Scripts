...
public class Shotgun : MonoBehaviour, IBasicWeapon, IBasicBulletShooter, IShotgunBlaster, IAmmoUser
{
	[Header("Components")]
	...
	[SerializeField] private GameObject blastPellets = default;
	...
	public void InstantiateProjectile()
	{
		...
		{
			GameObject projectileInstance = Instantiate(GetProjPref(), mouth.transform.position, Quaternion.identity, GetPjtsf());
	...
	public void CreateMuzzleFlash()
	{
		if (GetMuzzleFlash())
		{
			GameObject flash = Instantiate(GetMuzzleFlash(), mouth.transform.position, Quaternion.identity, GetMouth().transform);
			GameObject pellets = null;
			if (blastPellets)
				pellets = Instantiate(blastPellets, mouth.transform.position, Quaternion.identity, GetPjtsf());
			aimProjectile(flash);
			if (pellets != null)

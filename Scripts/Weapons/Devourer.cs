...
public class Devourer : MonoBehaviour, ICostingWeapon, IBasicWeapon, IRepeatingBlaster, IChargingWeapon
{
	[Header("Components")]
	...
	public static int GetDevCost() { return devcost; }
	...
	public void Update()
	{
		SetUIAmmoString(GetStatLibrary().GetSouls() > devcost ? "Shot ready" : "Shot unavailable");
	...
	public virtual void BeginFireProjectile()
	{
		...
		CreateMuzzleFlash();
		CameraShaker.Instance.ShakeOnce(2, 2, 0.2f, 0.2f);
	}
	...
	public void DoShoot()
	{
		if (GetStatLibrary().GetSouls() > devcost)
		{
			StartCoroutine(ReleaseTheBoys());
			GetStatLibrary().SetSouls(GetStatLibrary().GetSouls() - devcost);
			devcost += 750;
			isFiring = true;
		}
		else
